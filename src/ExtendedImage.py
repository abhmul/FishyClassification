...
def random_channel_shift(x, intensity, channel_index=0, **kwargs):
...
def standardize(x,
    ...
    if samplewise_center:
        x = x - np.mean(x, axis=samplewise_standardize_axis, keepdims=True)
    if samplewise_std_normalization:
        x = x / (np.std(x, axis=samplewise_standardize_axis, keepdims=True) + 1e-7)
...
def load_bounding_box(bounding_boxes, filename, index, from_dir):
    if from_dir:
        bounding_box = bounding_boxes[filename]
    else:
        bounding_box = bounding_boxes[index]
    x_offset, y_offset = bounding_box["x"], bounding_box["y"]
    h, w = bounding_box["height"], bounding_box["width"]
    return x_offset, y_offset, h, w


def roi_crop(x, bounding_boxes=None, filename=None, index=None, from_dir=True, dim_ordering=K.image_dim_ordering(), **kwargs):
    x_offset, y_offset, h, w = load_bounding_box(bounding_boxes, filename, index, from_dir)
    if dim_ordering == 'tf':
        return x[x_offset:x_offset + w, y_offset:y_offset + h, :]
    else:
        return x[:, x_offset:x_offset + w, y_offset:y_offset + h]


def roi_center(x, bounding_boxes=None, filename=None, index=None, from_dir=True, dim_ordering=K.image_dim_ordering(),
               fill_mode='nearest', cval=0, **kwargs):
    channel_index = 2 if dim_ordering == 'tf' else 0
    ...
    transform_matrix = translation_matrix  # no need to do offset
    x = apply_transform(x, transform_matrix, channel_index=channel_index, fill_mode=fill_mode, cval=cval)
    return x
...
class ImageDataGenerator(object):
    ...
    def process(self, x, filename=None, index=None):
        # get next sync_seed
        ...
        for p in self.__pipeline:
            x = p(x, filename=filename, index=index, **self.config)
        return x
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            x = self.X[j]
            x = self.image_data_generator.process(x, index=j)
            if i == 0:
    ...
    def next(self):
        ...
        if self._reader_generator_mode:
            ...
            for i, j in enumerate(index_array):
                ...
                if x.ndim == 2:
                    x = np.expand_dims(x, axis=0)
                x = self.image_data_generator.process(x, filename=name)
                if i == 0:
                    batch_x = np.zeros((current_batch_size,) + x.shape)
                batch_x[i] = x
