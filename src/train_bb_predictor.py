...
Xtr = np.load("/input/train_imgs.npy")
masktr = np.load("/input/train_masks.npy")
...
if DEBUG:
    ...
    im, msk = next(train_generator)
    for i in range(len(im)):
        ax[0, 0].imshow(im[i])
        ax[0, 1].imshow(msk[i, :, :, 0], cmap="gray")
        ax[1, 0].imshow(im[i] * msk[i])
        array_to_img(im[i] * msk[i]).save('out_{}.png'.format(i))
        plt.pause(.5)
    im, msk = next(val_generator)
    for i in range(len(im)):
        ax[0, 0].imshow(im[i])
        ax[0, 1].imshow(msk[i, :, :, 0], cmap="gray")
        ax[1, 0].imshow(im[i] * msk[i])
